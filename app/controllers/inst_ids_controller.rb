...
# Instructure services.  InstIDs expire after one hour.  Canvas hands out
# encrypted tokens that need to be decrypted by the API Gateway before they can
# be accepted by Canvas or other services.
#
# @model InstID
#    {
#      "id": "InstID",
...
class InstIdsController < ApplicationController
  ...
  #
  # @example_request
  #   curl 'https://<canvas>/api/v1/inst_ids' \
  #         -X POST \
  ...
  def create
    ...
    increment_request_cost(Setting.get("inst_id_token_additional_request_cost", "200").to_i)
