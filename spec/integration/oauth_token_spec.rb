...
%w[Twitter].each do |integration|
  describe integration do
    ...
    def oauth_start(integration)
      ...
      expect_any_instance_of(OAuth::Consumer).to receive(:token_request).and_return({ oauth_token: "test_token", oauth_token_***REMOVED***, authorize_url: "http://oauth.example.com/start" })
      ...
      it "creates the UserService on successful auth" do
        ...
        if integration == "Twitter"
          expect(Twitter::Connection).to receive(:from_request_token).and_return(double("TwitterConnection",
                                                                                        access_token: double("AccessToken", token: "test_token", ***REMOVED***),
