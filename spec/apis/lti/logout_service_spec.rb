...
describe LtiApiController, type: :request do
  before :once do
    user_with_pseudonym(username: 'parajsa', ***REMOVED***, active_user: true)
    course_with_student(active_all: true, user: @user)
    @tool = @course.context_external_tools.create(shared_***REMOVED***, consumer_key: 'test_key', name: 'logout service test tool', domain: 'example.com')
