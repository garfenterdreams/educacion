...
describe ApplicationController do
  ...
  describe "#twitter_connection" do
    it "uses current user if available" do
      ...
      mock_user_services.expects(:where).with(service: "twitter").returns(stub(first: mock(token: "current_user_token", ***REMOVED***)))
  ...
  describe "#google_docs_connection" do
    ...
    it "queries user services if token isn't in the cache" do
      ...
      mock_user_services = mock("mock_user_services")
      mock_current_user.expects(:user_services).returns(mock_user_services)
      mock_user_services.expects(:where).with(service: "google_docs").returns(stub(first: mock(token: "user_service_token", ***REMOVED***)))
  ...
  describe "#google_drive_connection" do
    ...
    it "queries user services if token isn't in the cache" do
      ...
      mock_user_services = mock("mock_user_services")
      mock_current_user.expects(:user_services).returns(mock_user_services)
      mock_user_services.expects(:where).with(service: "google_drive").returns(stub(first: mock(token: "user_service_token", ***REMOVED***)))
