...
require_relative "../shared_examples/module2_item_selective_release_assign_to_shared_examples"
...
describe "selective_release module item assign to tray", :ignore_js_errors do
  ...
  context "using assign to tray for newly created items", :ignore_js_errors do
    ...
    it "shows the correct icon type and title a new assignment" do
      go_to_modules
      module_header_expand_toggles.first.click
      add_newly_created_item("Assignment", @module, "New Assignment Title")
    ...
    it "shows the assign to option for newly-created items that a teacher can manage" do
      ...
      module_item = ContentTag.last
      manage_module_item_button(module_item.id).click
      expect(module_item_action_menu_link_exists?("Assign To...")).to be_truthy
      ...
      module_item = ContentTag.last
      manage_module_item_button(module_item.id).click
      expect(module_item_action_menu_link_exists?("Assign To...")).not_to be_truthy
    end
  end
  ...
  context "assign to tray values" do
    ...
    it "shows tray and Everyone pill when accessing tray for an item that has no overrides" do
      ...
      open_assign_to_tray_for(@module_item1.id)
    ...
    it "shows points possible only when present", :ignore_js_errors do
      ...
      click_cancel_button
      manage_module_item_button(@module_item2.id).click
      click_manage_module_item_assign_to
    ...
    context "differentiation tags" do
      ...
      it "Displays inherited differentiation tags from module" do
        # Create a module override for differentiation tag
        go_to_modules
        module_action_menu(@module.id).click
        ...
        expect(assign_to_in_tray("Remove #{@diff_tag1.name}")[0]).to be_displayed
        expect(inherited_from[0].text).to eq("Inherited from Module 1")
      end
      ...
      context "differentiation tag rollback" do
        ...
        shared_examples_for "Convertable in Assign To Tray" do
          ...
          it "removes error message when user manually removes all differentiation tag overrides and can save" do
            ...
            expect(element_exists?(convert_override_button_selector)).to be_falsey
  ...
  context "assign to tray focus validation" do
    ...
    it "focuses on trash can button on newly created card" do
      go_to_modules
      open_assign_to_tray_for(@module_item1.id)
  ...
  context "item assign to tray saves", :ignore_js_errors do
    ...
    it_behaves_like "module2 item assign to tray", :context_modules
    it_behaves_like "module2 item assign to tray", :course_homepage
  end

  # Canvas for Element is not supported with the Module Teacher View yet but this should be uncommented
  ...
  context "permissions" do
    ...
    def assert_permission_toggles_item_visibility(item, permission)
      go_to_modules
      open_assign_to_tray_for(item.id)
