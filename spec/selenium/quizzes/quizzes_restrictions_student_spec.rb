...
describe 'quiz restrictions as a student' do
  ...
  context 'restrict access code' do
    ...
    it 'requires an access code', priority: "1", test_id: 345735 do
      begin_taking_quiz
      expect(fj("input[type=password][name= 'access_code']")).to be_present
    end

    it 'allows you to enter a correct access token password to view the quiz', priority: "1", test_id: 345734 do
      begin_quiz(@password)
      expect(f('.quiz-header')).to include_text 'Quiz Instructions'
    end

    it 'does not allow you to enter an incorrect access token password to view the quiz', priority: "1", test_id: 338079 do
      begin_quiz('lechuck')
  ...
  context 'filter ip addresses' do
    ...
    it 'is not accessible from invalid ip address', priority: "1", test_id: 338081 do
      begin_taking_quiz
